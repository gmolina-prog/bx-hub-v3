      ...
      {!loading && viewMode === 'kanban' && (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-5 gap-3">
          {STAGES.map(stage => {
            ...
            const stageTheme = {
              indicacao:    { header: '#EEF2FF', border: '#DDD6FE', dot: '#818CF8', text: '#4338CA', badge: '#6366F1' },
              documentacao: { header: '#FFF7ED', border: '#FED7AA', dot: '#FB923C', text: '#9A3412', badge: '#EA580C' },
              comite:       { header: '#FEFCE8', border: '#FEF08A', dot: '#EAB308', text: '#713F12', badge: '#CA8A04' },
              aprovado:     { header: '#F0FDF4', border: '#BBF7D0', dot: '#22C55E', text: '#14532D', badge: '#16A34A' },
              liberado:     { header: '#EFF6FF', border: '#BFDBFE', dot: '#60A5FA', text: '#1E40AF', badge: '#2563EB' },
            }
            const th = stageTheme[stage.id] || stageTheme.indicacao
            return (
              <div key={stage.id}
                className={`flex flex-col rounded-2xl border-2 transition-all ${dragOverStage === stage.id ? 'border-violet-400 ring-2 ring-violet-300 ring-offset-2' : 'border-zinc-200'}`}
                style={{ background: '#F8F9FC' }}
                onDragOver={e => onDragOver(e, stage.id)}
                onDragLeave={onDragLeave}
                onDrop={e => onDrop(e, stage.id)}
              >
                <div className="px-4 py-3 rounded-t-xl border-b" style={{ background: th.header, borderColor: th.border }}>
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2">
                      <div className="w-2.5 h-2.5 rounded-full" style={{ background: th.dot }} />
                      <span className="text-xs font-bold text-zinc-700">{stage.label.replace(/^\d+ - /, '')}</span>
                      <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-white border" style={{ color: th.text, borderColor: th.border }}>{list.length}</span>
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-bold text-zinc-600">{formatCurrency(stageValue)}</span>
                    <span className="text-[10px] text-zinc-400">pond. {formatCurrency(weightedValue)}</span>
                  </div>
                </div>
                <div className="p-3 space-y-3 flex-1 min-h-[160px]">
                  {list.length === 0 ? (
                    ...
                    const accentColor = item.cover_color || th.dot
                    const urgency = isStale
                      ? { label: `Parado · ${aging.label}`, bg: '#FEF2F2', color: '#B91C1C', border: '#FECACA' }
                      : isWarning
                      ? { label: `Atenção · ${aging.label}`, bg: '#FFFBEB', color: '#D97706', border: '#FDE68A' }
                      : { label: `Ativo · ${aging.label}`, bg: '#F0FDF4', color: '#16A34A', border: '#BBF7D0' }
                    const probColor = prob >= 80 ? '#22C55E' : prob >= 50 ? '#6366F1' : '#F59E0B'
                    const probTextColor = prob >= 80 ? '#16A34A' : prob >= 50 ? '#4338CA' : '#D97706'
                    ...
                    return (
                      <div key={item.id} draggable
                        ...
                        className={`bg-white rounded-xl cursor-pointer transition-all overflow-hidden ${draggingId === item.id ? 'opacity-30 scale-95' : 'hover:shadow-lg hover:-translate-y-0.5'}`}
                        style={{
                          boxShadow: isStale ? '0 0 0 1.5px #FECACA' : '0 1px 3px rgba(0,0,0,0.07), 0 0 0 0.5px rgba(0,0,0,0.05)',
                          borderLeft: `3px solid ${isStale ? '#EF4444' : accentColor}`,
                        }}
                      >
                        <div className="p-4">
                          {/* 1 — Badge urgência */}
                          ...
                          {/* 2 — EMPRESA */}
                          <div className="text-sm font-bold text-zinc-800 mb-1 leading-snug">
                            {item.entity_name || <span className="italic font-normal text-zinc-400 text-xs">Sem empresa</span>}
                          </div>
                          {/* 3 — Linha de captação */}
                          <div className="text-xs text-zinc-500 mb-2 leading-snug line-clamp-2">{item.name || '—'}</div>
                          {/* 4 — Instituição */}
                          {item.institution_name && (
                            <div className="flex items-center gap-1.5 mb-3">
                              <div className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: accentColor + 'AA' }} />
                              <span className="text-[11px] text-zinc-400">{item.institution_name}</span>
                            </div>
                          )}
                          {/* 5 — Valor + ponderado */}
                          <div className="flex items-end justify-between mb-2">
                            <div>
                              ...
                              <div className="text-base font-bold" style={{ color: isStale ? '#DC2626' : th.badge }}>{formatCurrency(item.value)}</div>
                            </div>
                            <div className="text-right">
                              <div className="text-[9px] text-zinc-400 uppercase tracking-wider font-bold mb-0.5">Ponderado</div>
                              <div className="text-sm font-bold text-zinc-500">{formatCurrency(weighted)}</div>
                            </div>
                          </div>
                          {/* 6 — Prob bar */}
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className="text-[10px] font-bold w-7 text-right shrink-0" style={{ color: probTextColor }}>{prob}%</span>
                          </div>
                          {/* Checklist */}
                          {checklistTotal > 0 && (
                            <div className="flex items-center gap-2 mb-3">
                              <div className="flex-1 h-1 bg-zinc-100 rounded-full overflow-hidden">
                                <div className="h-full rounded-full bg-emerald-500" style={{ width: `${checklistTotal > 0 ? checklistDone/checklistTotal*100 : 0}%` }} />
                              </div>
                          ...
                          {item.next_action && (
                            <div className="rounded-lg px-3 py-2 mb-3"
                              style={{ background: isStale ? '#FEF2F2' : '#F9FAFB', borderLeft: `2px solid ${isStale ? '#FCA5A5' : accentColor + '88'}` }}>
                              <div className="text-[9px] uppercase tracking-wider font-bold mb-1" style={{ color: isStale ? '#B91C1C' : '#9CA3AF' }}>Próxima ação</div>
                              <div className="text-[11px] leading-snug line-clamp-2" style={{ color: isStale ? '#7F1D1D' : '#374151' }}>{item.next_action}</div>
                            </div>
                          )}
                          {/* 8 — Footer */}
                          <div className="flex items-center justify-between">
                            {assigned ? (
                              <div className="flex items-center gap-1.5">
                                <div className="w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-bold text-white shrink-0"
                                  style={{ background: assigned.avatar_color || '#6366F1' }}>
                                  {(assigned.initials || assigned.full_name?.slice(0,2) || '?').toUpperCase()}
                                </div>
                                <span className="text-[11px] text-zinc-500">{assigned.full_name?.split(' ')[0]}</span>
                              </div>
                            ) : (
                            ...
                            {item.expected_close && (
                              <span className="text-[10px] text-zinc-400">
                                Fecha: {new Date(item.expected_close).toLocaleDateString('pt-BR', { month: 'short', year: '2-digit' })}
                              </span>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="border-t border-zinc-100 flex">
                          {prevStage ? (
                            <button onClick={e => { e.stopPropagation(); moveItem(item.id, prevStage.id) }}
                              className="flex-1 text-[10px] py-2 px-3 text-zinc-400 hover:bg-zinc-50 hover:text-zinc-600 transition-colors text-center">
                              ← {prevStage.label.replace(/^\d+ - /, '').slice(0, 12)}
                            </button>
                          ) : <div className="flex-1" />}
                          {prevStage && nextStage && <div className="w-px bg-zinc-100" />}
                          {nextStage ? (
                            <button onClick={e => { e.stopPropagation(); moveItem(item.id, nextStage.id) }}
                              className="flex-1 text-[10px] py-2 px-3 font-bold transition-colors text-center hover:bg-zinc-50"
                              style={{ color: th.badge }}>
